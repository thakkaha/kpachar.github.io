...
var registerEventListeners = function(){
    $('#grid').on('selectstart', function(e){return false;});
...
var processCell = function(cell, doNotMark){
    ...
    for(colIdx=cellColIdx; colIdx < N_COLS; colIdx++){
        matchCandidate = matchCandidate + $('#tblCell_'+cellRowIdx+'_'+colIdx).text();
    ...
    for(diagIdx=0; diagIdx+cellRowIdx < N_ROWS && diagIdx+cellColIdx < N_COLS; diagIdx++){
        ...
        if(convertToArrayOfAksharas(matchCandidate).length>1 && isProperMatch(matchCandidate)){
            ...
            markMatch(cellRowIdx, cellColIdx, (diagIdx+cellRowIdx), (diagIdx+cellColIdx), matchCandidate);
        }
    }
    return matches;
}
...
var markMatch = function(startRowIdx, startColIdx, endRowIdx, endColIdx, word){
    ...
    for(cellIdx = 0; cellIdx < endColIdx - startColIdx + 1; cellIdx++){
        var cellId = 'tblCell_'+(startRowIdx+cellIdx)+'_'+(startColIdx+cellIdx);
        markCell(cellId, word);
    }
}
...
var putWord = function(strWord){
    ...
    while(attemptedSlots.length < emptySlots.length){
        ...
        nextDirection:
        while(directionsRemaining.length > 0){
            ...
            for(wordIdx = 0; wordIdx < word.length; wordIdx++){
                ...
                switch(direction) {
                    case 'H':
                        targetCell = "#tblCell_"+slotRowIdx+"_"+(slotColIdx+wordIdx);
                        break;
                    case 'V':
...
"ध्येयनिष्ठा", 
"स्थिरास्तुन:",
"सघंशक्तिर्विजेत्रीयं", 
"कृतवास्मध्दर्मरक्षणम्",
"परमं",
"वैभवं",
"प्राप्तुं", 
"समर्थास्तु", 
